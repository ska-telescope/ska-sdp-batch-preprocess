...
def test_distributed_t_avg(test_data: xr.Dataset, client: Client):
    ...
    assert time_distributor.avg_time(5, 0.5).equals(averaging_time(test_data, 5, flag_threshold=0.5))
    assert freq_distributor.avg_time(5, 0.5).equals(averaging_time(test_data, 5, flag_threshold=0.5))
